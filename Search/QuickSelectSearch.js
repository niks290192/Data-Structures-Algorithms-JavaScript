/**
 * Places the `k` smallest element in `array` in the first `k` indices: `[0..k-1]`
 ...
 * - All element before index `k` are smaller than the `k`the element 
 * 
...
function quickSelectSearch(array, k) {
    ...
    while (from < to) {
        let left = from
        let right = to
        const pivot = array[Math.cell((left + right) * 0.5)]
